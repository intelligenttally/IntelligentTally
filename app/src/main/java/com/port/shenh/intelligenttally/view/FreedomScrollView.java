package com.port.shenh.intelligenttally.view;
 ...
 * 全方向滚动布局
 */
public class FreedomScrollView extends FrameLayout {
    ...
    protected float getRightFadingEdgeStrength() {
        ...
        final int span = computeChildRealSize(false, 0) - getScrollX() -
                rightEdge;
        if (span < length) {
    ...
    protected float getBottomFadingEdgeStrength() {
        ...
        final int span = computeChildRealSize(true, 0) - getScrollY() -
                bottomEdge;
        if (span < length) {
            return span / (float) length;
        }
    ...
    private boolean canScrollV() {
        ...
        if (child != null) {
            int childHeight = computeChildRealSize(true, 0);
    ...
    private boolean canScrollH() {
        View child = getChildAt(0);
        if (child != null) {
            int childWidth = computeChildRealSize(false, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (getChildCount() > 0) {
            ...
            if (child.getMeasuredHeight() < height || child.getMeasuredWidth() < width) {
                ...
                int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(height, MeasureSpec
                        .EXACTLY);
    ...
    private boolean inChild(int x, int y) {
        if (getChildCount() > 0) {
            ...
            final View child = getChildAt(0);
            return !(y < computeChildRealSize(true, -1) - scrollY || y >= computeChildRealSize
                    (true, 1) - scrollY ||
                    x < computeChildRealSize(false, -1) - scrollX || x >= computeChildRealSize
                    (false, 1) - scrollX);
        }
        return false;
    }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (left < viewRight && viewLeft < right) {
                ...
                if (focusCandidate == null) {
                    ...
                    final boolean viewIsCloserToBoundary = (leftFocus && viewLeft <
                            focusCandidate.getLeft()) || (!leftFocus && viewRight >
    ...
    public boolean arrowScrollV(int direction) {
        ...
        if (nextFocused != null && isWithinDeltaOfScreenV(nextFocused, maxJump, getHeight())) {
            ...
            if (direction == View.FOCUS_UP && getScrollY() < scrollDelta) {
                ...
                if (getChildCount() > 0) {
                    int daBottom = computeChildRealSize(true, 1);
    ...
    public boolean arrowScrollH(int direction) {
        ...
        if (nextFocused != null && isWithinDeltaOfScreenH(nextFocused, maxJump)) {
            ...
            if (direction == View.FOCUS_LEFT && getScrollX() < scrollDelta) {
                ...
                int daRight = computeChildRealSize(false, 1);
    ...
    public void smoothScrollBy(int dx, int dy) {
        ...
        if (duration > ANIMATED_SCROLL_GAP) {
            ...
            final int bottom = computeChildRealSize(true, 1);
            final int maxY = Math.max(0, bottom - height);
            final int scrollY = getScrollY();
            dy = Math.max(0, Math.min(scrollY + dy, maxY)) - scrollY;

            final int width = getWidth() - getPaddingRight() - getPaddingLeft();
            final int right = computeChildRealSize(false, 1);
    ...
    protected int computeVerticalScrollRange() {
        ...
        return computeChildRealSize(true, 0);
    }
    ...
    protected int computeHorizontalScrollRange() {
        ...
        return computeChildRealSize(false, 0);
    }
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (getChildCount() > 0) {
                ...
                x = clamp(x, getWidth() - getPaddingRight() - getPaddingLeft(),
                        computeChildRealSize(false, 0));
                y = clamp(y, getHeight() - getPaddingBottom() - getPaddingTop(),
                        computeChildRealSize(true, 0));
                super.scrollTo(x, y);
    ...
    protected int computeScrollDeltaToGetChildRectOnScreenV(Rect rect) {
        ...
        // bottom
        if (rect.bottom < computeChildRealSize(true, 0)) {
        ...
        if (rect.bottom > screenBottom && rect.top > screenTop) {
            ...
            int bottom = computeChildRealSize(true, 1);
    ...
    protected int computeScrollDeltaToGetChildRectOnScreenH(Rect rect) {
        ...
        if (rect.right < computeChildRealSize(false, 0)) {
        ...
        if (rect.right > screenRight && rect.left > screenLeft) {
            ...
            // make sure we aren't scrolling beyond the end of our content
            int right = computeChildRealSize(false, 1);
    ...
    public void fling(int velocityX, int velocityY) {
        if (getChildCount() > 0) {
            ...
            int right = computeChildRealSize(false, 0);

            int height = getHeight() - getPaddingBottom() - getPaddingTop();
            int bottom = computeChildRealSize(true, 0);
            mScroller.fling(getScrollX(), getScrollY(), velocityX, velocityY, 0, Math.max(0,
                    right - width), 0, Math.max(0, bottom - height));
    ...
    public void scrollTo(int x, int y) {
        ...
        if (getChildCount() > 0) {
            View child = getChildAt(0);
            x = clamp(x, getWidth() - getPaddingRight() - getPaddingLeft(), computeChildRealSize
                    (false, 0));
            y = clamp(y, getHeight() - getPaddingBottom() - getPaddingTop(), computeChildRealSize
                    (true, 0));
    ...
    /**
     * 计算子控件真实大小（由于使用setScale缩放导致的大小失真）
     *
     * @param vertical 是否使用ScaleY值计算，true使用ScaleY计算，false使用ScaleX计算
     * @param type     计算的数据类型，-1表示计算左边界或上边界，1表示计算右边界或下边界，0表示计算宽或高
     *
     * @return 真实大小
     */
    private int computeChildRealSize(boolean vertical, int type) {
        if (getChildAt(0) != null) {

            View child = getChildAt(0);

            int realSize = vertical ? (int) (child.getHeight() * child.getScaleY() + .5f) : (int)
                    (child.getWidth() * child.getScaleX() + .5f);

            switch (type) {
                case 0:
                    return realSize;
                case -1:
                    return vertical ? child.getTop() : child.getLeft();
                case 1:
                    return realSize;
            }
        }
